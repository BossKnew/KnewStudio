...
export default function JobHistory({ conversation, sourceAsset, onDeleteConversation, onUseAsReference, onOpenImage, onRetry }: JobHistoryProps) {
  ...
  return <section className="jobs stack">
    <div className="jobs-heading"><h2>{conversation.title}</h2><button className="button danger" type="button" onClick={onDeleteConversation}>删除会话</button></div>
    {conversation.jobs.map((job) => <article className="card stack job-card" key={job.id}>
      <div className="job-heading"><div className="row"><strong>{job.modelSnapshot.displayName}</strong><span>{modeLabel(job.mode)}</span><span className="muted">{job.status}</span></div>{job.status === 'FAILED' && <button className="button" type="button" disabled={retryingJobId === job.id} onClick={() => void retry(job.id)}>{retryingJobId === job.id ? '正在重试…' : '重试'}</button>}</div>
      ...
      <div className="job-images">{job.assets.map((jobAsset) => {
        ...
        return <div className={`image-card job-image-card ${sourceAsset?.id === jobAsset.id ? 'selected-reference' : ''}`} key={jobAsset.id}>
          <button className="image-thumbnail" type="button" onClick={() => onOpenImage(referenceAsset)} aria-label="放大查看生成图片"><img src={jobAsset.contentUrl} alt={job.prompt} /><span className="image-expand" aria-hidden="true">放大</span></button>
          <button className="button reference-button" type="button" onClick={() => onUseAsReference(referenceAsset, job.prompt)}>{sourceAsset?.id === jobAsset.id ? '已设为参考图' : '设为参考图'}</button>
        </div>;
...
function modeLabel(mode: string) {
  if (mode === 'IMAGE_EDIT') return '整图编辑';
...
function DeletedAssetPlaceholder() {
  return <div className="deleted-asset-placeholder" role="status"><span aria-hidden="true">▧</span><strong>已在资产库中删除</strong></div>;
}
