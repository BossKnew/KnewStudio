...
describe('AuthService MFA login state machine', () => {
  async function fixture(user: any) {
    ...
    return { service: new AuthService(prisma as any, redis as any, limits as any, mfa as any), password, tx, mfa, limits };
  }
  ...
  it('allows an optional user without MFA and writes only the v2 session namespace', async () => {
    const { service, password, tx } = await fixture({ id: 'u2', username: 'normal', role: 'USER', status: 'ACTIVE', mustChangePwd: false, mfaCredential: null });
