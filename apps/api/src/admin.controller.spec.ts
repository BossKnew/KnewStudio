...
describe('AdminController user statistics', () => {
  it('counts only visible, non-deleted library assets', async () => {
    const prisma: any = {
      user: { findMany: jest.fn().mockResolvedValue([{ id: 'user-1', username: 'alice', _count: { jobs: 4, conversations: 2 } }]) },
      asset: { groupBy: jest.fn().mockResolvedValue([{ userId: 'user-1', _count: { _all: 3 }, _sum: { sizeBytes: 4096n } }]) },
    };
    const controller = new AdminController(prisma, {} as any, {} as any, {} as any, { releaseJob: jest.fn().mockResolvedValue(undefined) } as any);

    const result = await controller.users();

    expect(prisma.asset.groupBy).toHaveBeenCalledWith(expect.objectContaining({ where: { role: { in: ['UPLOAD', 'OUTPUT'] }, deletedAt: null } }));
    expect(result[0]).toMatchObject({ _count: { jobs: 4, conversations: 2, assets: 3 }, storageBytes: '4096' });
  });
  ...
  it('uses the shared cancellation flow when disabling a user', async () => {
    ...
    const quota = { releaseJob: jest.fn().mockResolvedValue(undefined) };
    const prisma: any = {
    ...
    const controller = new AdminController(prisma, auth as any, {} as any, queue as any, quota as any);

    await controller.userStatus({ id: 'admin-1' } as any, 'user-1', { status: 'DISABLED' });

    expect(queue.getJobs).toHaveBeenCalledWith(['waiting', 'delayed', 'prioritized', 'paused'], 0, -1, true);
    ...
    expect(quota.releaseJob).toHaveBeenCalledTimes(2);
  });
});
